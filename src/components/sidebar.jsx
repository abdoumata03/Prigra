...
const Sidebar = () => {
  ...
  return (
    <div className="h-full flex flex-col justify-start items-center py-10 w-1/5 bg-white">
      ...
      <div className="flex flex-col items-center justify-center mb-5">
        ...
        />
        <h1 className="font-semibold text-gray1 mb-1">MATALLAH Abdallah</h1>
        <p className="text-sm text-gray3 mb-10">a.matallah@esi-sba.dz</p>
      </div>
        ...
        {StudentSidebarData.map((val, index) => (
          ...
          >
            <NavLink to={val.link} activeclassname="active">
              <li className={`flex flex-row gap-4 items-center ${selectedItemIndex === index ? `text-white` : `text-gray1`} `}>
                <div>{React.cloneElement(val.icon, {fill: selectedItemIndex === index? 'white': '#333333' })}</div>
                <div>{val.title}</div>
              </li>
            </NavLink>
          </button>
